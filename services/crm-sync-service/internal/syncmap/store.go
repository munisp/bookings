...
// ddl is the idempotent bootstrap statement (SPEC-CRM §B schema).
...
func (s *Store) Get(ctx context.Context, kind, opendeskID string, tenantID *uuid.UUID) (Mapping, error) {
	...
	q := `SELECT id, kind, opendesk_id, twenty_id, tenant_id, updated_at FROM sync_map
     WHERE kind = $1 AND opendesk_id = $2 AND tenant_id = $3`
	err := s.pool.QueryRow(ctx, q, kind, opendeskID, tid).Scan(&m.ID, &m.Kind, &m.OpenDeskID, &m.TwentyID, &m.TenantID, &m.UpdatedAt)
...
// Put inserts or updates (kind, opendesk_id, tenant_id) -> twenty_id.
func (s *Store) Put(ctx context.Context, kind, opendeskID, twentyID string, tenantID *uuid.UUID) error {
	_, err := s.pool.Exec(ctx, `INSERT INTO sync_map (kind, opendesk_id, twenty_id, tenant_id, updated_at)
		VALUES ($1, $2, $3, $4, now())
		ON CONFLICT (kind, opendesk_id, tenant_id)
		DO UPDATE SET twenty_id = EXCLUDED.twenty_id, updated_at = now()`,
